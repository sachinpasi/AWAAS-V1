...
const CookieBox = ({ isLoading }) => {
  ...
  return (
    <div
      ...
      } w-3/4 h-24 bg-green fixed  left-2/4 transform -translate-x-2/4 z-40 rounded-2xl transition-all duration-1000 shadow-2xl`}
    >
      <div className="flex w-11/12 h-full mx-auto flex-col justify-center items-center">
        <MdClose
          onClick={() => setIsOpen(false)}
          className="text-white text-3xl m-2 cursor-pointer absolute right-0.5 top-0.5"
        />
        <div className="flex justify-between items-center">
          ...
          <button
            onClick={() => setIsOpen(false)}
            className="text-xl font-medium  bg-white text-green px-8 py-2 rounded"
          >
            Accept
          </button>
        </div>
